 ...
 *        json]
 *        P1 ---> F[(backend_update_euro_values)]
 *        F ---> R[returns
 *        json]
 *        R ---> C
 *        ```
...
drop function if exists backend_update_euro_values;

create or replace function backend_update_euro_values (
...
begin
    ...
    for var_rec_day in
        ...
        select
            eur.idf_euro,
            to_char (to_date (eur.idf_euro :: text, 'yyyymmdd') - '1 day' :: interval, 'yyyymmdd') :: numeric "num_previous",
            eur.num_value
        from
            dat_euros eur
        where
