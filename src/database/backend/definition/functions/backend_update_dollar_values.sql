 ...
 *        json]
 *        P1 ---> F[(backend_update_dollar_values)]
 *        F ---> R[returns
 *        json]
 *        R ---> C
 *        ```
...
drop function if exists backend_update_dollar_values;

create or replace function backend_update_dollar_values (
...
begin
    ...
    for var_rec_day in
        ...
        select distinct
            to_char ((json ->> 'date') :: date, 'yyyymmdd') :: numeric "idf_dollar",
    ...
    for var_rec_day in
        ...
        select
            usd.idf_dollar,
            to_char (to_date (usd.idf_dollar :: text, 'yyyymmdd') - '1 day' :: interval, 'yyyymmdd') :: numeric "num_previous",
            usd.num_value
        from
            dat_dollars usd
        where
