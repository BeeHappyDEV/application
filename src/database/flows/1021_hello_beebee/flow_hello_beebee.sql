 ...
 *        autonumber
 *        Actor Chatbot
 *        participant F as flow_hello_beebee
 *        Chatbot ->> F: in_jsn_transfer
 *        F ->> F: utils_get_first_name
 *        F ->> F: idf_expression: 1021
 *        F -->> Chatbot: out_jsn_transfer
 *        ```
 */

drop function if exists flow_hello_beebee;

create or replace function flow_hello_beebee (
...
begin
    ...
    var_jsn_outgoing = core_set_json_numeric (var_jsn_outgoing, 'idf_expression', 1021);
