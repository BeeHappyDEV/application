drop function if exists frontend_collaborator_page;

create or replace function frontend_collaborator_page (
...
begin
    ...
    if (var_txt_last_name = '') then
        ...
        select
            '+' || substring (var_txt_phone_2, 1, 2) || ' ' || substring (var_txt_phone_2, 3, 3) || ' ' || substring (var_txt_phone_2, 6, 3) || ' ' || substring (var_txt_phone_2, 9, 3)
        into
            var_txt_phone_1;
    ...
    var_jsn_outgoing = core_set_json_text (var_jsn_outgoing, 'txt_phone', var_txt_phone_1);
