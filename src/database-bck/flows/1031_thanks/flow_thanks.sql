 ...
 *        autonumber
 *        Actor Chatbot
 *        participant F as flow_thanks
 *        Chatbot ->> F: in_jsn_transfer
 *        F ->> F: utils_get_first_name
 *        F ->> F: idf_expression: 1031
 *        F -->> Chatbot: out_jsn_transfer
 *        ```
 */

drop function if exists flow_thanks;

create or replace function flow_thanks (
...
begin
    ...
    var_jsn_outgoing = core_set_json_numeric (var_jsn_outgoing, 'idf_expression', 1031);
