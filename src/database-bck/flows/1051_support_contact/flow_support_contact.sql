 ...
 *        autonumber
 *        Actor Chatbot
 *        participant F as flow_support_contact
 *        Chatbot ->> F: in_jsn_transfer
 *        F ->> F: utils_get_first_name
 *        F ->> F: select constants: company_support
 *        F ->> F: select constants: company_call
 *        F ->> F: idf_expression: 1051
 *        F -->> Chatbot: out_jsn_transfer
 *        ```
 */

drop function if exists flow_support_contact;

create or replace function flow_support_contact (
...
begin
    ...
    var_jsn_outgoing = core_set_json_numeric (var_jsn_outgoing, 'idf_expression', 1051);
